...
> = {
  ...
  video: ({ value, setValue }) => (
    <FileInputContainer
      value={value as File}
      setValue={setValue as React.Dispatch<React.SetStateAction<File>>}
    />
  ),
  image: ({ value, setValue }) => (
    <FileInputContainer
      value={value as File}
      setValue={setValue as React.Dispatch<React.SetStateAction<File>>}
    />
...
const UserInputContainer = ({ type }: { type: InputTypes }) => {
  ...
  const inputMap: Record<
    InputTypes,
    UserInputProps<string> | UserInputProps<File>
  > = {
  ...
  async function simplifyContent() {
    ...
    const response = await fetch(
      `${env.BACKEND_URL}/api/v1/${classifyMap[type]}`,
      {
        method: "POST",
        headers,
        body: requestBody,
  ...
  return (
    <div className="flex flex-col">
      ...
      <span className="mt-6 mx-auto">
        <Button variant="submit" onClick={simplifyContent}>
          Simplify!
        </Button>
      </span>
    </div>
